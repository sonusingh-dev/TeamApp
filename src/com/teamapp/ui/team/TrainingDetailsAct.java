...
public class TrainingDetailsAct extends TeamMenuAct {
	...
	private TrainingDetailsAct mTraining;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		lblPresent.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (!mPresentPlayerList.isEmpty() && !mRoleId.equals("4")) {
					Intent intent = new Intent().setClass(mTraining,
		...
		lstComments.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (userId.equals(mUserId)) {
					...
					TeamAppAlerts.showMessageDialog(mTraining, message);
				}
			}
		});
		...
		btnGmap.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent = new Intent().setClass(mTraining, GMapAct.class);
		...
		btnPost.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (message.length() == 0) {
					// VoetBalAlerts
					// .showToast(mTraining, "Enter some comments...");
					return;
				}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Log.d("onActivityResult", "executing...");
	...
	private void populateAttendance() {
		...
		lstPresent.setAdapter(new SimpleAdapter(mTraining, mPresentPlayerList,
				R.layout.player_present, new String[] { "userName" },
				new int[] { R.id.lblPresent }));

		lstAbsent.setAdapter(new SimpleAdapter(mTraining, mAbsentPlayerList,
				R.layout.player_absent, new String[] { "userName" },
				new int[] { R.id.lblAbsent }));

		lstSupporting.setAdapter(new SimpleAdapter(mTraining,
				mSupporterPlayerList, R.layout.player_supporting,
				new String[] { "userName" }, new int[] { R.id.lblSupporting }));

		lstNotResponded
				.setAdapter(new SimpleAdapter(mTraining,
						mNotRespondedPlayerList, R.layout.player_not_responded,
						new String[] { "userName" },
	...
	private void populateComments() {
		...
		CommentsAdapter mAdapter = new CommentsAdapter(mTraining, mImages);
	...
	private void checkCalSetting() {
		...
		int calSetting = mPreferences.getInt("Training", 0);
		...
		case ONLY_IF_PRESENT:
			...
			if (!isPresent) {
				TeamAppAlerts.showMessageDialog(mTraining,
		...
		if (Utility.getEventFromCalendar(mTraining, mActivity)) {
			TeamAppAlerts.showMessageDialog(mTraining,
					getString(R.string.calendar_already));
			return;
		}

		if (Utility.addEventToCalendar(mTraining, mActivity)) {
			TeamAppAlerts.showMessageDialog(mTraining,
	...
	private void getActivities() throws Exception {
		...
		request.addProperty("bTraining", true);
	...
	private void getActivityDetails() throws Exception {
		...
		SoapObject request = WebServiceHelper.getSOAPRequest(METHOD_NAME);
		request.addProperty("trainingId", mId);
		request.addProperty("teamId", mTeamId);
		request.addProperty("clubId", mClubId);
		SoapObject response = (SoapObject) WebServiceHelper.getSOAPResponse(
				request, METHOD_NAME);
		...
		if (response != null) {
			...
			String subType = response.getProperty("TrainingType").toString()
					.trim();
			...
			String dateTime = response.getProperty("TrainingDateTime")
					.toString();
	...
	private void setAttendance(HashMap<String, String> player, String attTo,
		...
		try {
			...
			if (status) {
				...
				TeamAppAlerts.showToast(mTraining,
	...
	private void showDeletionDialog(final String id, final int index,
		...
		AlertDialog.Builder optionBuilder = new AlertDialog.Builder(mTraining);
	...
	private void showEditCommentsDialog(final HashMap<String, String> comments,
		...
		final EditText txtComments = new EditText(mTraining);
		txtComments.setLines(4);
		txtComments.setGravity(Gravity.TOP);
		txtComments.setText(comments.get("message"));

		AlertDialog.Builder builder = new AlertDialog.Builder(mTraining);
		...
		builder.setPositiveButton(getString(R.string.update),
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (message.length() == 0) {
							// VoetBalAlerts
							// .showToast(mTraining, "Enter comments");
							return;
						}
		...
		builder.setPositiveButton("Send",
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						String message = txtComments.getText().toString();
						if (message.length() == 0) {
							TeamAppAlerts.showToast(mTraining, "Enter Message");
							return;
						}
	...
	private void showAttendancePopup(final View view,
		...
		LayoutInflater inflater = LayoutInflater.from(mTraining);
	...
	private void runActivity() {
		...
		final Handler handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 1:
					progressBar.setVisibility(View.GONE);
					TeamAppAlerts.showToast(mTraining, getString(R.string.no)
	...
	private void runDeleteActivity() {
		...
		TeamAppAlerts.showToast(mTraining, getString(R.string.deleting));
		progressBar.setVisibility(View.VISIBLE);
		new Thread() {
			@Override
			public void run() {
	...
	private void runSendNotification(final String message, final String method) {
		...
		TeamAppAlerts.showToast(mTraining, getString(R.string.sending));
		new Thread() {
			@Override
			public void run() {
	...
	private void runPostComments(final String message) {
		...
		TeamAppAlerts.showToast(mTraining, getString(R.string.posting));
		new Thread() {
			@Override
			public void run() {
